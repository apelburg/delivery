...
INSERT INTO `".DOSTAVKA_BIG_ROW_TBL."` (`id`, `num_rows`, `status`, `date`, `parent_id_address`, `target_typpe`, `target`, `actions`, `docs`, `date_delivery`, `contacts`, `disable_editing`) VALUES ('NULL', '$add_num_rows', 'off', '$date', '', '', '', '', '', '', '',''); ";
	...
	switch($address_data)
	{
		...
		case 'clients':
		/*
			$query = "SELECT `c_l`.`id`,`c_l`.`company`,`c_l`.`phone`,`c_l`.`addres`,`c_l`.`delivery_address`,`rel_clm`.`client_id`,`rel_clm`.`manager_id`,`rel_ccf`.`name` AS `cf_name`, `rel_ccf`.`department` AS `cf_phone1`,`rel_ccf`.`phone` AS `cf_phone2`,`rel_ccf`.`set_main` AS `cf_set_main` FROM `os__client_list` AS `c_l`
INNER JOIN `order_manager__relate_client_manager_tbl` AS `rel_clm` ON `c_l`.`id`=`rel_clm`.`client_id`
LEFT JOIN `order_manager__relate_client_cont_faces` AS `rel_ccf` ON `rel_ccf`.`id` = if((SELECT COUNT(*) FROM `order_manager__relate_client_cont_faces` AS `tbl_4` WHERE `tbl_4`.`client_id` = `c_l`.`id`)>1,(SELECT `id` FROM `order_manager__relate_client_cont_faces` AS `tbl_4` WHERE `tbl_4`.`client_id` = `c_l`.`id` AND `tbl_4`.`set_main` = 'on'),(SELECT `id` FROM `order_manager__relate_client_cont_faces` AS `tbl_4` WHERE `tbl_4`.`client_id` = `c_l`.`id`)) ";
*/
$query = "SELECT `c_l`.`id`,`c_l`.`company`,`c_l`.`phone`,`c_l`.`addres`,`c_l`.`delivery_address`,`rel_clm`.`client_id`,`rel_clm`.`manager_id`,`rel_ccf`.`name` AS `cf_name`, `rel_ccf`.`department` AS `cf_phone1`,`rel_ccf`.`phone` AS `cf_phone2`,`rel_ccf`.`set_main` AS `cf_set_main` FROM `os__client_list` AS `c_l`
INNER JOIN `os__client_manager_relation` AS `rel_clm` ON `c_l`.`id`=`rel_clm`.`client_id`
LEFT JOIN `os__client_cont_faces_relation` AS `rel_ccf` ON `rel_ccf`.`id` IN (SELECT `id`
FROM `os__client_cont_faces_relation` AS `tbl_4`
WHERE `tbl_4`.`client_id` = `c_l`.`id`
AND `tbl_4`.`set_main` = 'on') ";

if($access!=1){$query .= " WHERE `rel_clm`.`manager_id`= $amanager_id ";}else{$query .= " ";}
			// echo $query;
			// echo '*1*';
			// return;
			$result = mysql_query($query,$db);
			...
			if(mysql_num_rows($result) > 0){// >
				while($item = mysql_fetch_assoc($result)){
					echo '<div class="row2" onClick="getThisAddress(this,\''.$id_row.'\')"><div class="cell2" data-id="'.$item['id'].'">'.$item['company'].'</div><div class="cell2">'; 
					if($item['delivery_address']!=""){echo 'Адрес: '.$item['delivery_address'].'<br/> ';}
					if($item['phone']!=""){echo 'тел.: '.$item['phone'].'<br/> ';}
					echo $item['cf_name'].' '.$item['cf_phone1'].' '.$item['cf_phone2'].'</div></div>';
				}
			}else{
	...
	//echo '<div class="row2" data-cors="'.$address_data.'"><div class="cell2"><input type="text" style="width:90%;"></div><div class="cell"><input type="text"><select><option value="0" selected>выберите...</option><option value="suppliers">поставщик</option><option value="clients">клиент</option></select><input type="button" style=" margin-right: 60px;" value="добавить" onClick="addNewAddressCorS(this)"></div></div>';
